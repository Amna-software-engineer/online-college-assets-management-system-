...
<div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-500">
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h2 className="text-2xl md:text-3xl font-black text-[#1E2D44] italic uppercase tracking-tighter">Centralized Reports</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">College-wide Inventory Audit</p>
        </div>

        <CSVLink data={filteredAssets} filename={`College_Report_${view}.csv`} className="w-full md:w-auto">
            <button className="w-full md:w-auto bg-[#008BA9] text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:shadow-lg transition-all">
    ...
    <div className="bg-white rounded-[2.5rem] p-4 shadow-sm border border-slate-100 flex flex-col md:flex-row md:flex-wrap items-stretch md:items-center justify-between gap-4">
        <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            {reportTabs.map((tab) => (
                <button
                    key={tab.id}
                    onClick={() => setView(tab.id)}
                    className={`flex items-center gap-2 px-4 md:px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap shrink-0 ${view === tab.id
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto border-t md:border-t-0 pt-4 md:pt-0">
            <div className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                <Filter size={14} className="text-[#008BA9] shrink-0" />
                <select
    ...
    <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-5 md:p-8 border-b border-slate-50 bg-slate-50/20 flex flex-col md:flex-row md:justify-between gap-4">
            ...
            <CSVLink data={csvData} filename={`${view}-assets-report.csv`} className="w-full md:w-auto">
                <button className="w-full md:w-auto bg-[#008BA9] text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 cursor-pointer hover:bg-[#008BA9]/90 transition-all">
